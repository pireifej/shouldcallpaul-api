...
app.post('/getDailyBreadAudio', async (req, res) => {
  try {
    ...
    const audioPath = path.join(DEVOTIONAL_AUDIO_DIR, `daily_bread_${date}.mp3`);
    if (fs.existsSync(audioPath)) {
      console.log(`🔊 Daily Bread audio served from disk for ${date}`);
      const audioBuffer = fs.readFileSync(audioPath);
      dailyBreadAudioCache.set(date, audioBuffer); // warm the memory cache
...
app.post('/editRequest', authenticate, (req, res) => {
